...
<h2>Deterministic, with an agent on top</h2>
<p>The numbers come from code, not the model. The engine builds the load curve and prices it the same way every time, so the figures are checkable and don't drift. The agent only reads the inputs, your words or a menu photo, and explains the result. It never does the maths. The model can misread a menu, but it can't invent a kWh or a price.</p>
